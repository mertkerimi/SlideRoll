...
struct Strings {
    ...
    var allowPhotos: String  { L(["Allow Photos Access","Fotoğraflara İzin Ver","Permitir acceso a Fotos","Autoriser l'accès aux photos","Consenti accesso alle foto","Permitir acesso às fotos","Fotozugriff erlauben","写真へのアクセスを許可","사진 접근 허용","允许访问照片","السماح بالوصول للصور","Разрешить доступ к фото"]) }
    ...
    var ob1Title: String { L(["Welcome to PhotoMint","PhotoMint'a Hoş Geldin","Bienvenido a PhotoMint","Bienvenue sur PhotoMint","Benvenuto su PhotoMint","Bem-vindo ao PhotoMint","Willkommen bei PhotoMint","PhotoMintへようこそ","PhotoMint에 오신 것을 환영합니다","欢迎使用 PhotoMint","مرحبًا بك في PhotoMint","Добро пожаловать в PhotoMint"]) }
    ...
    var weeklyReminderSub: String    { L(["Reminds you every week on the chosen day","Her hafta belirtilen günde hatırlatır","Te recuerda cada semana el día elegido","Vous rappelle chaque semaine le jour choisi","Ti ricorda ogni settimana nel giorno scelto","Lembra você toda semana no dia escolhido","Jede Woche am gewählten Tag erinnern","毎週指定した曜日に通知","매주 선택한 요일에 알림","每周在所选日期提醒你","يذكّرك كل أسبوع في اليوم المحدد","Напоминает каждую неделю в выбранный день"]) }
